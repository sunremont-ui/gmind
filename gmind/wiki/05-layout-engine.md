...
Создаётся по якорю узла: **левый клик** по точке → сразу ребёнок в её сторону; **правый клик** → меню «Направление» (`AnchorActionMenu`); драг в пустоту → ребёнок в сторону драга. Всё сходится в `MindMap.createChildInDirection`. Персист: backend `model.Topic.ChildDir`.

## Рёбра дерева (направление, толщина, цвет)

`renderer.tsx` рисует ребро родитель→ребёнок с учётом геометрии (обновлено 2026-06-08):

- **Точки примыкания по стороне ребёнка** — `edgeEndpoints(parent, child)` выбирает грань (право/лево/низ/верх), `edgePath(..., axis)` гнёт кривую по нужной оси. Раньше всегда «правая грань родителя → левая ребёнка».
- **Толщина = размер поддерева** — `thicknessForSubtree(sizeMap[id])`: ствол (много потомков) толще листьев (1.5→~7.5px).
- **Цвет = вес ребра** — `topic.edge_weight` через `weightToColor` (холодный синий → горячий красный, HSL 220°→0°). Без веса — цвет темы. Контрол «Edge Weight» в PropertiesPanel; backend `model.Topic.EdgeWeight`.

## Бейджи числа детей + посторонний fold (folded_sides)

- На каждом узле — кружок с числом детей в каждом направлении, где есть дети (`childBadges`, сторона по `sideOf`). У выбранного узла их заменяют интерактивные якоря.
- **Клик по бейджу сворачивает только эту сторону** — `topic.folded_sides: string[]` (`onToggleChildSide`). Скрытие БЕЗ переракладки: дети исчезают на местах, соседи и кружки не двигаются. Свёрнутая сторона — полый кружок. Backend `model.Topic.FoldedSides`.
- Общий счётчик детей с узла убран — общее число только в PropertiesPanel.

## Floating-узлы с поддеревом

Floating-топики раскладываются как самостоятельные корни поддерева (`MindMap.floatingLayouts` → `renderer` через `floatingRoots`), поэтому узел, «оторванный» в пустоту (drag тела узла → detach), сохраняет видимыми всех детей. Раньше floating рендерился одиночным листом.

## Drag тела узла (swap / child / detach)

`handlePointerMove/Up` + `dropZone`: центр цели → поменять местами (`swapTopics`); край цели → стать ребёнком в ту сторону (`moveTopic` + `child_dir`, активная точка цели увеличивается); пустота → свободный floating с поддеревом (`detachToFloating`). Backend: `POST /topics/{id}/swap`, `POST /topics/{id}/detach`.
