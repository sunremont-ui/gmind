...
>
> **Живое состояние здесь не ищите.** Чем работа держится сейчас — решения,
> гейты, открытые хвосты и следующий разрешённый шаг — лежит в треке GM слоя
> лабы (`node lab.mjs brief`). Файл остаётся хронологией с обоснованием: у
> абзаца нет способа узнать, что мир изменился, и он не может возразить, когда
> устарел. Невыполненные пункты ниже — след своей сессии, а не список работ:
> аудит 2026-09-02 показал, что из 74 таких пунктов механически проверяемы 23,
> и 8 из них давно сделаны.

---

## Сессия: 2026-08-29 — Корневая навигация и файловое дерево

### Что просили

После перехода по ссылке из карты проекта в другой файл сохранить понятный путь
назад, показать дерево исходного корня, управлять его раскрытием, создавать и
удалять документы, менять ширину Sidebar и вернуть удачную тёмную тему с
логотипом из превью.

### Что сделано

- shell-level история документов: кнопки назад/вперёд и `Alt+←/→`;
- `DocumentContextBar`: breadcrumb корня, возврат к карте корня, reveal файла;
- `ProjectTree` в Sidebar: только `.md`/`.markdown`/`.xmind` и папки-предки;
- папки по умолчанию свёрнуты, есть «свернуть всё» и «развернуть всё»;
- создание Markdown в корне/папке и удаление документа с подтверждением;
- `POST`/`DELETE /api/v1/projects/files` с canonical-path confinement,
  защитой от перезаписи и пересканированием корневого workbook;
- drag-resize Sidebar 220–560px, клавиатурное управление, сброс к 260px и
  persistence в `gmind_sidebar_width`;
- `midnight` стал темой карты по умолчанию, выбор хранится в `gmind_theme`;
- Midnight включает dark color-scheme и `/lumen-logo-dark.svg` в Header,
  Splash и экране ошибки.

### Проверка реализации

- frontend: 40 test files, 428 tests — зелёные;
- production build — успешно;
- backend: `go test ./...` — все пакеты ok;
- browser smoke: активен Midnight, canvas тёмный, тёмный логотип загружается,
  переходы документов сохраняют корневой контекст.

Подробная архитектурная запись: `wiki/18-project-root-navigation.md`.

---

## Сессия: 2026-08-27 — Ошибки отображения, тесты и схема проектов

### Что просили
Починить ошибки отображения (узлы/текст, связи, наложения, панели), сделать
тесты — в том числе на оптимизацию — и научить Gmind показывать схему своих
проектов; форматов `.md` и `.xmind` вместе достаточно.

### 1. Ошибки отображения (все найдены тестами и закрыты тестами)

| # | Дефект | Где | Как чинилось |
|---|---|---|---|
| 1 | Группы разных направлений одного родителя накладывались (ветка «вверх» на «вверх-влево») | `renderer/layout.ts` | `separateGroups()` — разведение групп вдоль их же направлений |
| 2 | Ветка, растущая назад, накрывала своего предка | `renderer/layout.ts` | глобальный проход двигает поддерево потомка |
| 3 | Карты крупнее 300 узлов вообще не чистились от наложений (проход отключался по размеру) | `renderer/layout.ts` | проход переведён на равномерную сетку, лимит снят |
| 4 | Узел, зажатый между двумя соседями, качался между ними до конца проходов и оставался поверх | `renderer/layout.ts` | `escapeAll()` — уход сразу от всех соседей, без разворота назад |
| 5 | Жирный `rich_text` мерился обычным начертанием → текст обрезался по `overflow: hidden` | `renderer/nodeFont.ts` (новый) | единый резолвер шрифта для раскладки и `TopicNode` |
| 6 | Двунаправленная связь рисовала обе стрелки в одну сторону | `MindMap/RelationshipLine.tsx` | один маркер с `auto-start-reverse` на оба конца |
| 7 | Связь со своим цветом получала наконечник цвета типа | там же | `fill="context-stroke"` |
| 8 | Nav Rail: с ростом числа модулей нижние кнопки и «Настройки» уезжали за край | `NavRail/NavRail.tsx` | список модулей прокручивается, «Настройки» вне списка |

Раскладка: **3109 узлов — 13 мс, ноль наложений** (было: выше 300 узлов проход
не выполнялся вовсе).

### 2. Тесты

Фронтенд 360 → **413**, бэкенд +3 пакета с тестами:

- `renderer/layoutOverlap.test.ts` — инварианты «карта читается»: 8 направлений,
  ветка назад, случайные карты по 5 структурам, карта >300 узлов, свёрнутые
  узлы, ножки рёбер не заходят внутрь узлов;
- `renderer/layoutPerf.test.ts` — бюджеты: ~3000 узлов, линейность роста
  (4× узлов ≠ 16× времени), кэш измерений;
- `renderer/nodeFont.test.ts`, `MindMap/RelationshipLine.test.tsx`,
  `NavRail/NavRail.test.tsx`, `ProjectsPanel/ProjectsPanel.test.tsx`,
  `utils/openTopicLink.test.ts`;
- Go: `internal/project` (12 тестов + 2 бенчмарка + потолки времени/узлов),
  `internal/xmind` (был **без единого теста**), `internal/api/projects_test.go`.
- `src/test-setup.ts` — localStorage в памяти: jsdom в этой сборке отдаёт
  заглушку без методов, и код с localStorage в тестах не проверялся.

### 3. Схема проектов (каталог → карта)

`backend/internal/project/scan.go` + эндпоинты `/api/v1/projects/{scan,import,
open-doc,dirs}` + модуль **Проекты** в Nav Rail. Подробности — в AGENTS.md,
раздел «Схема проекта: каталог → карта».

**Побочно найдено и исправлено:** экспорт в `.xmind` терял тело узла,
оформление, вид памяти, ссылку и всю типизацию связей (та же потеря данных, что
чинили в `.md` коммитом 23794a2). Добавлено поле `gmind` в узле и связи —
round-trip без потерь, совместимость с обычным XMind сохранена.

### Проверка
`go vet ./...` — чисто, `go test ./...` — все пакеты ok, `tsc -b --force` —
чисто, `eslint` — 0 ошибок (122 старых предупреждения), `vitest` — 413/413.

---

## Сессия: 2026-07-30 — Двусторонняя работа с памятью MASys + библиотека компонентов

### Контекст
«Gmind — это холст: в нём можно просто работать как раньше, но главное — работа с
памятью MASys и библиотека визуальных компонентов.» Холст не тронут, добавлены
два слоя поверх.

### 1. Мост записи в память (`masys_write.go`, `masys_push.go`)

Было: только чтение + удаление. Стало:

| Path | tRPC | Назначение |
|---|---|---|
| `POST /masys/memory/episodes` | `memory.episode.log` | эпизод |
| `POST /masys/memory/remember` | `memory.controller.remember` | запомнить |
| `POST /masys/memory/entities/upsert` | `memory.entity.upsert` | сущность |
| `POST /masys/memory/relations` | `memory.kg.addRelation` | связь |
| `POST /masys/push` | оба | узлы+связи холста → граф |
| `POST /masys/runs/start` | `runs.start`/`runs.invoke` | поставить задачу |
| `POST /masys/runs/{id}/stop` | `runs.stop` | остановить |

**Гочи, найденные чтением схем MASys:**
- `runs.start` принимает только `pipelineId`; данные идут через `runs.invoke`
  (инжект в `pipeline-input`). Прокси сам выбирает метод.
- `entityTypeEnum` узкий (`person|place|org|concept|custom`) — виды памяти Gmind
  сводятся `normalizeEntityType()`, исходный вид сохраняется в
  `attributes.gmind_memory_kind`, иначе zod отклоняет запись целиком.

**Push (обратная сторона KG-sync):** уходят только узлы с `memory_kind`;
идентичность по `MasysRef.Key` → повторный push идемпотентен, переименование не
плодит дубли; связь уходит только если оба конца ушли; частичная неудача даёт
200 с отчётом (`errors[]`), а не рушит запрос. `Topic.MasysRunID` — карта помнит,
где поставлена работа.

UI: Workbench → таб «🔗 Узел» (`NodeMemoryActions.tsx`): запомнить, в граф,
эпизод, отправить выделение, поставить задачу.

### 2. Библиотека визуальных компонентов

`renderer/componentLibrary.ts` — каталог заготовок. Корпус = «как выглядит вид
памяти», компонент = «что кладу на холст». Категории: Память (по слоям karp),
Структуры (радиальный хаб, циферблат, дерево решения, линия времени, матрица
2×2), Задачи, Заметки. Структурные компоненты сразу задают раскладку.

Реестр расширяемый как у корпусов (`setCustomComponents` + persist-стор).
`componentFromTopic()` сохраняет выделенный узел как компонент с поддеревом.
Палитра — модуль «🧩 Компоненты» в Nav Rail; вставка через `importTopicTree`
под выделенный узел.

**Баг, поймал тест:** id своего компонента строился из `Date.now()` — два
компонента в одну миллисекунду получали один id, и один молча затирал другой в
реестре. Добавлен монотонный счётчик.

### Проверка — завершена в сессии 2026-08-27
`go vet`, `go test ./...`, `tsc -b --force`, `eslint`, полный vitest — зелёные
(см. сессию 2026-08-27 ниже). Заметка о непрогнанной сборке снята.

---

## Сессия: 2026-07-29 (2) — Визуальный язык памяти: корпуса узлов + раскладки 360°

### Контекст
Направление уточнено: Gmind — мозг визуализации памяти и место ручной доработки,
MASys — мозг связи со всеми системами и многослойной памяти. Первый шаг —
визуальный язык: разные виды памяти и разные роли узлов должны отличаться
визуально, «как корпуса радиокомпонентов», плюс таблица этих корпусов.

### 1. Корпуса памяти — `renderer/memoryPackages.ts`

Корпус = форма + цвет + тип обводки + насыщенность заливки + код-маркировка.
Формы выбраны осмысленно: `note` для working (черновик, пунктир+прозрачный),
`parallelogram` для episodic (скос = время), `hexagon` для procedural (гайка),
`rectangle` для artifact (коробка), `ellipse`+двойная обводка для meta,
`diamond` для decision (ромб блок-схемы), `pill` для conversation (реплика).

- `resolveNodeSkin()` — приоритет: **явная настройка узла → корпус → тема**.
  Смена `memory_kind` не затирает ручное оформление.
- **Реестр расширяемый:** `setCustomPackages()` внедряет пользовательские
  корпуса из `store/memoryPackages.ts` (Zustand + persist в localStorage).
  Свой корпус может переопределить встроенный; `isOverridden`/`builtinPackage`
  дают метку «изменён» и сброс. Функции реестра чистые и синхронные — их зовёт
  рендер узла, тесты идут без React.
- ⚠️ Стор импортируется в `App.tsx` намеренно: иначе свои корпуса появлялись бы
  только после открытия Workbench (rehydrate не запускался).

### 2. Роль узла: ветвление vs лист

`nodeRole()` → leaf/branch/hub (корень или ≥5 детей). `NodePins.tsx` рисует
выводы на сторонах, куда уходят дети — ветвление отличается силуэтом, а не
подписью. Свёрнутая сторона — бледные выводы. Хаб получает крепче обводку, но
заданную вручную толщину роль не трогает. Заодно вынесен `childSideCounts()` в
`edgeVisuals.ts` — renderer больше не считает стороны инлайном.

### 3. Таблица корпусов — Workbench, таб «📦 Корпуса»

`MemoryPackageTable.tsx` — доки + пикер + редактор: миниатюры рисуются тем же
`shapePath`, что и узлы; клик по строке ставит вид выделенному узлу; «+ Добавить
корпус», правка ✎ (у встроенного — создаёт переопределение), удаление 🗑 / сброс ↺.
`validatePackage()` проверяет ключ/название/код/форму/цвет.

### 4. Раскладки вокруг узла — `renderer/radialLayout.ts`

5 видов: `radial-even` (равные секторы), `radial-packed` (сектор по габариту),
`radial-rings` (кольцами, шахматкой), `radial-clock` (циферблат 12 позиций),
`radial-sector` (веер 180°). `radial` = историческое имя `radial-even`.
...
габаритов при любом угле. Полураствор = `asin((r+gap/2)/R)`, радиус подбирается
двоичным поиском (`fitRadius`). Старая `layoutRadial` (8 фиксированных
направлений + надежда на `resolveOverlaps`) удалена.

**Найденный тестом дефект:** шаг между кольцами считался только по крупнейшему в
текущем кольце → большой узел внешнего кольца садился на маленький внутреннего.
Фикс — `ringStep()` учитывает и крупнейшего среди ещё не размещённых.

Выбор вида: `PropertiesPanel` → Layout (с подсказкой) + контекстное меню узла.

### Проверка
- Vitest: +67 тестов (radialLayout 46, memoryPackages 21). Неперекрытие
  проверяется напрямую попарным сравнением bbox поддеревьев: 5 видов × 1…40
  детей, разнородные размеры, поддеревья с потомками, нулевой зазор.

---

## Сессия: 2026-07-29 — Markdown как рабочий формат + формы узлов + MASys из коробки

### Контекст
Три задачи одной сессией: (1) сделать проект рабочим с .md файлами,
(2) базовый набор форм узлов, (3) чтобы MASys подключался сразу.

### 1. Markdown как рабочий формат

**Backend** — новый пакет `internal/markdown` (Parse/Render, 17 тестов):
заголовок → голова узла, абзацы → тело (`body`), цитата → `notes`,
`<!-- gmind: {...} -->` → форма/иконка/цвета/`memory_kind`/`masys_ref`.

- `Topic.MdForm` (`heading` | `list`) — **ключевое**: без него сохранение
  переписывало авторские списки в заголовки. Узел, дописанный под пунктом
  списка, становится вложенным пунктом.
- Строка тела, похожая на `- пункт`/`# заголовок`/`> цитату`, экранируется `\`.
- `Workbook.SourcePath` + `SourceSyncedAt` — карта помнит свой файл.
- Эндпоинты: `POST /md/open`, `GET /md/files`, `POST /workbooks/{id}/md/save`,
  `POST /workbooks/{id}/md/reload`, `POST /workbooks/import/markdown`;
  `export/markdown` переписан на новый рендерер (раньше терял тело).
- Хранилище: `%APPDATA%\Gmind\markdown` (`MARKDOWN_PATH`).

**Frontend** — паритетный `utils/markdown.ts` (19 тестов) + `markdownExport.ts`,
`api/markdown.ts`, модуль **Markdown** в Nav Rail (`MarkdownPanel`).
`importTopicTree` теперь досылает body/notes/свойства вторым запросом.

### 2. Формы узлов

`renderer/shapes.ts` — единый реестр (46 тестов): `rounded`, `rectangle`,
`pill`, `ellipse`, `diamond`, `hexagon`, `parallelogram`, `note`, `cloud`.

- `TopicNode` рисует один `<path>` на любую форму (ромб больше не повёрнутый
  `rect`), `shapeTextInset` держит голову+тело внутри контура.
- `shapeGrow` в `layout.ts`: ромб/эллипс раздувают бокс под тот же текст.
- `PropertiesPanel/ShapePicker.tsx` — сетка миниатюр + живое превью вместо
  `<select>`; миниатюры рисуются тем же `shapePath`, что и узлы.

### 3. MASys из коробки

**Корневая причина:** MASys backend слушает `:5010`, а Gmind по умолчанию бил
в `:3001` (и фолбэк `:3000`) — связь не поднималась никогда.

- Дефолт → `http://localhost:5010`; `masys_status.go` — фоновый монитор:
  автопоиск по кандидатам, перепроверка каждые 20 с, кэш статуса
  (`/masys/health` больше не ждёт сетевой таймаут), `?refresh=1`.
- Найденный адрес прокидывается в `WorkerPool.SetMaSysBaseURL`.
- `PUT /api/v1/masys/config` — ручной адрес, пишется в `%APPDATA%\Gmind\masys.json`.
- Фикс tRPC: процедуры MASys требуют объект input даже при необязательных
  полях → пустую карту шлём как `input={}`; `limit` приводим к int (zod
  `z.number()` строку не принимает). До фикса `entities` отдавал ошибку.
- UI: `startHealthWatch()` в `App.tsx`, точка состояния на иконках Nav Rail,
  `MaSysConnection.tsx` в MaSysPanel (статус, latency, смена адреса).

### Проверка
- Go: все пакеты OK (новые: markdown 17 тестов, API md-файлы 7, MASys-монитор 7)
- Vitest: 147 → 212 тестов OK (markdown 19, shapes 46), `tsc --noEmit` чистый
- Живой прогон на реальном MASys (:5010): namespaces/entities/episodes/skills/
  wiki/decisions отдают данные; .md round-trip (открыть → дописать → сохранить)
  сохраняет списки, заголовки, тело и свойства узлов
